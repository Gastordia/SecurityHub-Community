...
Original: https://github.com/pqzx/html2docx — see that repository for its license.
Modified for this project to add support for more tags.
"""
import re, argparse
import io, os
...
def fetch_image(url, headers, base_url):
    ...
    `url` comes from user-controlled rich-text content (vulnerability
    description/POC/solution fields), so the resulting absolute URL is
    validated against the same SSRF allowlist used by PDF generation
    before any request is made — this also defeats the URL userinfo "@"
    bypass (e.g. src="@169.254.169.254/...") since that trick changes the
    parsed host, which is exactly what the allowlist check inspects.
    The bearer token is only attached when fetching from our own app
    (the getimage endpoint), never to an arbitrary whitelisted host.
    """
    full_url = base_url + url
    if not is_safe_fetch_url(full_url):
        return None
    try:
        request_headers = {}
        if "/api/project/getimage/" in full_url:
            request_headers["Authorization"] = f"Bearer {headers}"
        response = requests.get(
            full_url, headers=request_headers, verify=True,
...
class HtmlToDocx(HTMLParser):
    ...
    def handle_img(self, current_attrs):
        ...
        # fetch image — local (non-URL) image paths are not supported, only remote/relative URLs
        src_is_url = True
        if src_is_url:
            try:
