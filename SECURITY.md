...
We appreciate and encourage responsible disclosure of security vulnerabilities within this open-source project. If you discover any vulnerabilities or security issues, please report them privately using [GitHub's private security advisory feature](../../security/advisories/new) for this repository (do not open a public issue). We value your efforts to enhance the security of this project and kindly request that you provide details on the vulnerability discovered.
...
**Important Security Considerations for SecurityHub:**
...
- **High-Priority Vulnerabilities:** SecurityHub utilizes features that may lead to the following security concerns:
...
This ensures that:
- Entity resolution is disabled (`resolve_entities=False`)
- Network access is disabled (`no_network=True`)
- Huge trees are disabled (`huge_tree=False`)
- Recovery mode is disabled (`recover=False`)
